...
?>
<?php
// Result of a submitted popup form (register, change password, ...), shown in its own popup.
ob_start();
error_msg();
success_msg();
$account_message = ob_get_clean();
?>
<div id="account-modals">
    ...
    <div class="modal" id="account-message-modal">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body"><?php echo $account_message; ?></div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal"><?php elang('close'); ?></button>
                </div>
            </div>
        </div>
    </div>
    <script>
        document.addEventListener('DOMContentLoaded', function () { $('#account-message-modal').modal('show'); });
    </script>
    <?php } ?>
</div>
            ...
            <div class="col-lg-12 text-center" style="margin-top: -30px;">
                <?php if (!get_config('disable_online_players')) {
                    foreach (get_config('realmlists') as $onerealm_key => $onerealm) {
                        echo "<p><span style='color: #4db848;font-weight: bold;'>{$onerealm['realmname']}</span> <span style='font-size: 12px;'>(" . lang('online_players_msg1') . " " . user::get_online_players_count($onerealm['realmid']) . ")</span></p><hr>";
                        $online_chars = user::get_online_players($onerealm['realmid']);
                        if (!is_array($online_chars)) {
                            echo "<span style='color: #7cc576;'>" . lang('online_players_msg2') . "</span>";
                        } else {
        ...
        <div class="row">
            <div class="col-lg-12 text-center" style="margin-top: -30px;">
                <?php if (!get_config('disable_top_players')) {
                    $i = 1;
                    foreach (get_config('realmlists') as $onerealm_key => $onerealm) {
                        echo "<h6 data-aos='fade-up' data-aos-delay='100' style='color: #4db848;font-weight: bold;'>{$onerealm['realmname']}</h6><hr>";
